      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          ...
          <p className="text-(--secondary-text) text-lg max-w-xl">
            Anonymous visitor analytics. No cookies, no tracking scripts, no personal data collected.
          </p>
        </motion.div>
        ...
        {loading && !data ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
              <StatCard label="Page Views" value={data.totalViews} delay={0.1} />
              <StatCard label="Unique Visitors" value={data.uniqueVisitors} delay={0.15} />
              <StatCard
                label="Pages / Visit"
                value={data.uniqueVisitors > 0 ? (data.totalViews / data.uniqueVisitors).toFixed(1) : '0'}
                delay={0.2}
              />
              <StatCard label="Countries" value={data.countries.length} delay={0.25} />
            </div>
            ...
            >
              ...
              {data.dailyViews.length > 0 ? (
                ...
                <p className="text-(--secondary-text) text-center py-16">No data yet for this period.</p>
              )}
            </motion.div>

            {/* Two-column: Top Pages + Referrers */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
              <BarList title="Top Pages" items={data.topPages.map((p) => ({ label: p.path, value: p.views }))} delay={0.3} />
              <BarList title="Referrers" items={data.topReferrers.map((r) => ({ label: r.referrer, value: r.views }))} delay={0.35} />
            </div>

            {/* Three-column: Devices, Browsers, Countries */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
              <BarList title="Devices" items={data.devices.map((d) => ({ label: d.device, value: d.views }))} delay={0.4} />
              <BarList title="Browsers" items={data.browsers.map((b) => ({ label: b.browser, value: b.views }))} delay={0.45} />
              <BarList title="Countries" items={data.countries.map((c) => ({ label: c.country, value: c.views }))} delay={0.5} />
            </div>
            ...
            >
              No cookies · No IP addresses · No personal data · Aggregate only
            </motion.p>
          </>
        ) : null}
      </div>
